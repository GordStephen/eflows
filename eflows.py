...
for year in years:
    ...
    session = Session()
    production_flows_full = session.query(Flow).filter(Flow.source_node_name=='Primary Production', Flow.year==year, Flow.volume > norm_const/300).all()
    session.close()
...
print('Generating balance tables and final output...')
